...
项目当前处于首个无模型纵向切片：已经实现 Agent 名册、任务分派校验、运行状态投影，以及 Harness Host/Browser 插件骨架；尚未调用真实 Provider，也尚未提供生产可用的多 Agent 执行。
...
- Host 插件：提供 `agentTeam` 服务。
- Browser 插件：注册 Agent 面板和会话任务指挥台，使用 Emoji 专家头像与轻量动效。
