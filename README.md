...
项目当前处于无模型纵向切片：已经实现 Agent 名册、任务分派校验、运行状态投影，以及 Harness Host/Browser 插件骨架。Browser 面板会通过官方 Typert Remote API 读取 Host 的只读 PATH 扫描结果；尚未调用真实 Provider，也尚未提供生产可用的多 Agent 执行。
